...
time = ['10:00-11:00', '11:00-12:00', '12:00-13:00', '13:00-14:00', '16:00-17:00', '17:00-18:00', '18:00-19:00', '19:00-20:00', '20:00-21:00']
work_type = ['ПРО ягодицы', 'Здоровая спина', 'Скайстретчинг', 'Классическая растяжка', 'Растяжка с подкачкой', 
             'Пилатес', 'Йога', 'Йога в гамаках', 'ТРХ', 'Аэростретчинг', 'Аэродети', 'МФР', 'Детская растяжка', 'Ролл-стретчинг']
...
def get_coordinate():
    wb = load_workbook('/root/Bot/Raspisanie_trenirovok_2024.xlsx')
    sheet = wb.get_sheet_by_name('Январь 2024') # Поменять на функцию transform_date по наступлению января 2024
...
def write_cell(a, b):
    ...
    wb.save('/root/Bot/Raspisanie_trenirovok_2024.xlsx')
    wb.close
    ...
    try:
        if call.message:
            if call.data == 'Выгрузить файл':
                write_cell(excel_send_package[3], excel_send_package[4])  
                doc= open('/root/Bot/Raspisanie_trenirovok_2024.xlsx', 'rb')
                ...
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text = 'То, что ты выбирала:', reply_markup = None)               
